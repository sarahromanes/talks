...
.larger[
$$g(\mu_{ij}) = \eta_{ij} = \beta_0 + \boldsymbol{x}_i^T\boldsymbol{\beta}_j  \color{orange}{ + \boldsymbol{u}_i^T\boldsymbol{\lambda}_j }$$
]
...
.larger[
$$p(y_{ij}| \boldsymbol{u}_i, \boldsymbol{\Psi}) = \left\{ \frac{y_{ij} \eta_{ij} - b(\theta_{ij}) }{\phi_j} + c(y_{ij}, \phi_j)  \right\}.$$
]
...
## However, the integral needed to obtain the marginal likelihood proves to be intractable.
...
$$\begin{align}
      \ell(\boldsymbol{\Psi}) & = \sum_{i=1}^n \log \left\{p(\boldsymbol{y}_i, \boldsymbol{\Psi}) \right\} \\
                              & = \sum_{i=1}^n \log \left( \int \prod_{i=1}^{m} p(y_{ij}| \boldsymbol{u}_i,\boldsymbol{\Psi}) p(\boldsymbol{u}_i) d \boldsymbol{u}_i \right)
   \end{align}$$
]]

<img src="images/cool.gif", width="85%">
...
$$\begin{align}
      \underline{\ell}(\boldsymbol{\Psi}, \boldsymbol{\xi}) & = \sum_{i=1}^n \sum_{j=1}^{m}  \left\{ \frac{y_{ij} \widetilde{\eta}_{ij} - E_q\{b(\theta_{ij}) \}}{\phi_j} + c(y_{ij}, \phi_j)  \right\} \\
...
.medium[.black[
$$\widetilde{\eta}_{ij} = \beta_{0j} + \boldsymbol{x}_i^T\boldsymbol{\beta}_j  + \boldsymbol{\mu}_i^T\boldsymbol{\lambda}_j.$$  ]]
...
## .white[The genDA algorithm can be fit with the following syntax]



##### .orange[(*I expect the syntax will change slightly as I further improve the R package*)]
...
## Add acknowledgement to SSA NSW
